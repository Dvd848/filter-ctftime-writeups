    ...
    firebase.auth().onAuthStateChanged((user) => 
    {
        if (user) 
        {
            ...
            firebase.database().ref(`/data/${uid}/ctf_names`).once('value').then((snapshot) => 
            {
                ...
                const ctf_names = snapshot.val()?.split(WriteupFeedFilter.Constants.ENTRY_SEPARATOR) || [];
                ctf_names.length = Math.min(ctf_names.length, WriteupFeedFilter.Constants.MAX_CTF_ENTRIES); 
                ...
                $('#save_button').click(function()
                {
                    ...
                    const new_ctf_names = ctf_names_arr.join(WriteupFeedFilter.Constants.ENTRY_SEPARATOR);
